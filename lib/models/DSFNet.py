...
class ResASPPB(nn.Module):
    def __init__(self, channels):
        ...
        self.conv_3_1 = nn.Sequential(nn.Conv2d(channels, channels, 3, 1, 1, 1, bias=False), nn.LeakyReLU(0.1, inplace=True))
        self.conv_5_1 = nn.Sequential(nn.Conv2d(channels, channels, 3, 1, 4, 4, bias=False), nn.LeakyReLU(0.1, inplace=True))
        self.conv_7_1 = nn.Sequential(nn.Conv2d(channels, channels, 3, 1, 8, 8, bias=False), nn.LeakyReLU(0.1, inplace=True))
        self.conv_3_2 = nn.Sequential(nn.Conv2d(channels, channels, 3, 1, 1, 1, bias=False), nn.LeakyReLU(0.1, inplace=True))
        self.conv_5_2 = nn.Sequential(nn.Conv2d(channels, channels, 3, 1, 4, 4, bias=False), nn.LeakyReLU(0.1, inplace=True))
        self.conv_7_2 = nn.Sequential(nn.Conv2d(channels, channels, 3, 1, 8, 8, bias=False), nn.LeakyReLU(0.1, inplace=True))
        self.conv_3_3 = nn.Sequential(nn.Conv2d(channels, channels, 3, 1, 1, 1, bias=False), nn.LeakyReLU(0.1, inplace=True))
        self.conv_5_3 = nn.Sequential(nn.Conv2d(channels, channels, 3, 1, 4, 4, bias=False), nn.LeakyReLU(0.1, inplace=True))
        self.conv_7_3 = nn.Sequential(nn.Conv2d(channels, channels, 3, 1, 8, 8, bias=False), nn.LeakyReLU(0.1, inplace=True))
    ...
    def forward(self, x):
        buffer_1 = self.conv_3_1(x)
        buffer_1 = torch.cat((buffer_1, self.conv_5_1(x)), 1)
        buffer_1 = torch.cat((buffer_1, self.conv_7_1(x)), 1)
        buffer_1_ = self.b_1(buffer_1)

        buffer_2 = self.conv_3_2(buffer_1_)
        buffer_2 = torch.cat((buffer_2, self.conv_5_2(buffer_1_)), 1)
        buffer_2 = torch.cat((buffer_2, self.conv_7_2(buffer_1_)), 1)
        buffer_2_ = self.b_2(buffer_2)

        buffer_3 = self.conv_3_3(buffer_2_)
        buffer_3 = torch.cat((buffer_3, self.conv_5_3(buffer_2_)), 1)
        buffer_3 = torch.cat((buffer_3, self.conv_7_3(buffer_2_)), 1)
        buffer_3_ = self.b_3(buffer_3)
        return x + buffer_1_ + buffer_2_ + buffer_3_
...
class baseNet3D(nn.Module):
    ...
    def __init__(self, channel_input, channels):
        ...
        self.branch1 = nn.Sequential(
            ...
            # BasicConv3d(in_channels=64, out_channels=128, kernel_size=(3, 3, 3), stride=(1, 1, 1), padding=(1, 1, 1)),
        )
        ...
        scales = [2 ** i for i in range(len(channels[0:]))]
        self.dla_up = DLAUp(0, channels[0:], scales)
...
class DLASeg(nn.Module):
    ...
    def forward(self, x):
        ...
        #
        # layers = layersspatial
        layers = []

        for ii in range(3):
            # layers.append(layersspatial[ii]*layerstemporal[ii])
            # layers.append(torch.cat([layers1[ii], layerstemporal[ii]], dim=1))
...
def DSFNet(heads, head_conv=128):
...
# if __name__ == '__main__':
    # net = get_pose_net({'hm': 1, 'wh': 2, 'reg': 2}, 256).cuda()
    # from thop import profile
    # input  = torch.random(1,1,5,256,256).cuda()
