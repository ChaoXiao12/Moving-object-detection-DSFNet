...
class baseNet3D1(nn.Module):
    ...
    def __init__(self, channel_input, channels):
        ...
        self.branch4 = nn.Sequential(
            BasicConv3d(in_channels=channels[2], out_channels=channels[3], kernel_size=(1, 1, 3), stride=(1, 1, 1), padding=(0, 0, 1)),
            BasicConv3d(in_channels=channels[3], out_channels=channels[3], kernel_size=(1, 3, 1), stride=(1, 1, 1), padding=(0, 1, 0)),
            BasicConv3d(in_channels=channels[3], out_channels=channels[3], kernel_size=(3, 1, 1), stride=(1, 1, 1), padding=(1, 0, 0)),
            # BasicConv3d(in_channels=64, out_channels=128, kernel_size=(3, 3, 3), stride=(1, 1, 1), padding=(1, 1, 1)),
        )
        self.branch5 = nn.Sequential(
            BasicConv3d(in_channels=channels[3], out_channels=channels[4], kernel_size=(1, 1, 3), stride=(1, 1, 1), padding=(0, 0, 1)),
            BasicConv3d(in_channels=channels[4], out_channels=channels[4], kernel_size=(1, 3, 1), stride=(1, 1, 1), padding=(0, 1, 0)),
            BasicConv3d(in_channels=channels[4], out_channels=channels[4], kernel_size=(3, 1, 1), stride=(1, 1, 1), padding=(1, 0, 0)),
            # BasicConv3d(in_channels=64, out_channels=128, kernel_size=(3, 3, 3), stride=(1, 1, 1), padding=(1, 1, 1)),
        )
...
class DLASeg(nn.Module):
    def __init__(self, heads, final_kernel,
        ...
        channels = [16, 32, 64]  # self.base.channels
        scales = [2 ** i for i in range(len(channels[self.first_level:self.last_level]))]
        self.dla_up = DLAUp(self.first_level, channels[self.first_level:self.last_level], scales)
        channel_input = 3
        self.base3d = baseNet3D(channel_input, channels)
    ...
    def forward(self, x):
        ...
        layers = self.dla_up(layerstemporal)
...
def DSFNet_with_Dynamic(heads, head_conv=128):
