...
def _topk(scores, K=40):
    ...
    topk_scores, topk_inds = torch.topk(scores.view(batch, cat, -1), K)#沿着给定dim返回输入张量input中k个最大值
...
def ctdet_decode(heat, wh, reg=None, cat_spec_wh=False, K=100):
    #作用是将heat_map解码成b-box 将输出转化成det的函数
    batch, cat, height, width = heat.size()
    # heat = torch.sigmoid(heat)
    # perform nms on heatmaps
    
    #1.1首先经过_nms：
    #hmax用来寻找8-近邻极大值点，keep为h极大值点的位置，
    # 返回heat*keep，筛选出极大值点，为原值，其余为0。
    heat = _nms(heat)
